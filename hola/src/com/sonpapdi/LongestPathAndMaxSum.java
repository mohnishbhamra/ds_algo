...
public class LongestPathAndMaxSum {
...
// on the longest path from root to leaf node
    ...
    public static void main(String args[]) {
        ...
        System.out.println("Sum = "
                + sumOfLongRootWithMaxSum(root));
    }

    private static int sumOfLongRootWithMaxSum(Node root) {

        class MaxHeightMaxSumHelperClass {
            int maxSum = 0, maxH = 0;

            void helper(Node root, int localH, int localSum) {
                if (root == null) {
                    return;
                }
                localSum += root.data;
                ...
                helper(root.left, localH + 1, localSum);
                helper(root.right, localH + 1, localSum);
            }
        }
        MaxHeightMaxSumHelperClass maxHeightMaxSumHelperClassObject = new MaxHeightMaxSumHelperClass();
        maxHeightMaxSumHelperClassObject.helper(root, 1, 0);

        return maxHeightMaxSumHelperClassObject.maxSum;
    }
}
